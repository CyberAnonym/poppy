...
notes()
{
    echo -e "\033[034m$1$1 \033[0m"
}

#定义提示格式 #定义绿色字体输出success 红色字体是31m.绿色字是32m# 黄色颜色是33m
echo0()
{
...
echo1()
{ #定义红色字体输出failed
echo -e "$1$1 \033[031m [Failed] \033[0m"
}

#定义确认命令是否执行成功的消息输出的函数
checkLastCommand()
{
if [ $? -eq 0 ];then
    echo0 "$1"
...
install_tomcat()
{
    ...
    notes '现在开始往/usr/local/apache-tomcat-8.5.34/bin/catalina.sh里写入JAVA_HOME和JRE_HOME的值'
    sed -i '1a export JAVA_HOME=/usr/local/jdk1.8.0_191/' /usr/local/apache-tomcat-8.5.34/bin/catalina.sh
    sed -i '2a export JRE_HOME=/usr/local/jdk1.8.0_191/jre/' /usr/local/apache-tomcat-8.5.34/bin/catalina.sh
    checkLastCommand '环境变量写入完成' '失败'
}
