...
验证多用户访问（在 desktop0 上）：chihiro 可读写， 这
...
.. code-block:: bash
    ...
    expect eof
    eof
    touch /mnt/dev/poppy  #确认当前是否有权限


也可以切换到其他普通用户，试试是否能提升权限，这里因为要切换用户，所以不进行非交互式的方式，不一次性辅助粘贴全部。
...
.. code-block:: bash
    ...
    su - mike    //切换到普通用户
    su - chihiro
    cifscreds add -u chihiro server0    #向服务器提交用户认证凭据，密码是redhat
    touch /mnt/dev/b.txt     #确认有写入权限（新建文件)
